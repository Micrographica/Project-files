...
// Global store — loaded once, reused on every graph switch
let _allNodes = [];
let _allLinks = [];
let _activeSimulation = null; // track so we can stop it on switch
...
async function initGraph() {
  try {
    ...
    // ✅ Safety check
    if (!_allNodes?.length || !_allLinks?.length) {
      console.error("⚠️ Data not loaded correctly.");
      return;
    }

    console.log(`✅ Loaded ${_allNodes.length} nodes, ${_allLinks.length} links`);

    // Normalize IDs
    _allNodes.forEach(n => n.id = n.id.toString());
    _allLinks.forEach(l => {
      l.source = l.source.toString();
      l.target = l.target.toString();
    });

    // Default: start on Topics graph
    switchGraph("topic");
...
// --------------------------------------------------
// 🔄 Switch Graph — called by toggle buttons
// --------------------------------------------------

function switchGraph(groupKey) {
  // Stop any running simulation to avoid ghost forces
  ...
  d3.select("#abstract").html("<h2>Paper Abstract</h2><p>Click a node to view details.</p>");
  ...
  // Filter: always include nist nodes + the chosen group's nodes
  const filteredNodes = _allNodes
    .filter(n => n.group === "nist" || n.group === groupKey)
    .map(n => ({ ...n })); // shallow clone so simulation doesn't pollute original positions

  // Filter links to only those belonging to the active group
  const nodeIds = new Set(filteredNodes.map(n => n.id));
  const filteredLinks = _allLinks
    .filter(l => l.group === groupKey)
    .filter(l => nodeIds.has(l.source) && nodeIds.has(l.target))
    .map(l => ({ ...l, source: l.source.toString(), target: l.target.toString() }));

  console.log(`🔀 Switching to "${groupKey}" — ${filteredNodes.length} nodes, ${filteredLinks.length} links`);
...
function buildGraph(nodes, links) {
  ...
  // Expose simulation globally so switchGraph can stop it
  ...
  if (searchInput) {
    // Remove old listener by replacing node (avoids stacking listeners on switch)
